    ...
    if(isset($_FILES['image']['name'])){
        ...
        if(isset($_GET['rid'])){
            ...
            if(isset($path)){
                ...
                $query = "UPDATE `room` SET  `cost` = $cost, `location` = $location, `contact` = $contact, `name` = $name `description` =$desc
                WHERE `room`.`r_id` = $rid";
                mysqli_query($conn,$query);
            }else{
                $query = "UPDATE `room` SET `image`=$path `cost` = $cost, `location` = $location, `contact` = $contact, `name` = $name `description` =$desc
                WHERE `room`.`r_id` = $rid";
                mysqli_query($conn,$query);
            ...
            if(move_uploaded_file($_FILES['image']['tmp_name'],$path)){
                ...
                $query = "INSERT INTO `room` ( `r_id`,`o_id`,   `cost`, `location`, `description`, `contact`,`name`) 
...
<head>
    ...
    <title>Upload Room to Room Finder</title>
</head>
<body>
    <header>
        <div class="branding">
            <h1 class="app-name" ><span>R</span>oom <Span>F</Span>inder</h1>
        </div>
        <div class="user-info">
    ...
    <div class="outer">
        ...
        <form action="" method="post" enctype="multipart/form-data" class="shadow login-form">
            <div class="form-banner">Upload a Room</div>
...
<?php
    if(isset($_GET['rid'])){
        $rid = $_GET['rid'];
        $query = "SELECT * FROM `room` WHERE `r_id` = $rid";
